...
public class FakerParser implements PsiParser, LightPsiParser {
  ...
  /* ********************************************************** */
  // l1ParamDeclaration*
  public static boolean l1ParamList(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "l1ParamList")) return false;
    Marker m = enter_section_(b, l, _NONE_, L_1_PARAM_LIST, "<l 1 param list>");
    while (true) {
      int c = current_position_(b);
      if (!l1ParamDeclaration(b, l + 1)) break;
      if (!empty_element_parsed_guard_(b, "l1ParamList", c)) break;
    }
  ...
  /* ********************************************************** */
  // PARAMS_LIST_SEPARATOR l1ParamList
  ...
  /* ********************************************************** */
  // PARAMS_LIST_SEPARATOR l2ParamList
  ...
  /* ********************************************************** */
  // l2ParamDeclaration*
  public static boolean l2ParamList(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "l2ParamList")) return false;
    Marker m = enter_section_(b, l, _NONE_, L_2_PARAM_LIST, "<l 2 param list>");
    while (true) {
      int c = current_position_(b);
      if (!l2ParamDeclaration(b, l + 1)) break;
      if (!empty_element_parsed_guard_(b, "l2ParamList", c)) break;
    }
    exit_section_(b, l, m, true, false, null);
    return true;
  }
